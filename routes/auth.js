...
function generateAuthToken(user_id) {
    return jwt.sign({_id: user_id}, process.env.TOKEN_SECRET, {expiresIn: '15s'});
}
...
router.post('/login',async (req, res) =>{
    ...
    try {
        ...
        res.json({'auth-token': authToken, 'refresh-token': refreshToken});
    } catch (error) {
...
router.delete('/logout', verifyRefreshToken, async (req, res) =>{
    ...
    try {
        ...
        req.header('refresh-token', null);
        res.json({token: token, message: 'successfully logged out'});
    } catch (error) {
        res.status(500).json({message: error.message});
    }
})
